...
$(function(){
    ...
    $(document).on("click", "#btnEditTask", (e)=>{
        ...
        $.ajax({
            ...
            success: (task) => {
                var editDateStr = task[0].Date;
                $("#Edit_AppointmentId").val(task[0].AppointmentId);
                $("#Edit_Title").val(task[0].Title);
                $("#Edit_Description").val(task[0].Description);
                $("#Edit_Date").val(editDateStr.substring(editDateStr.indexOf("T"),0));
            }
        })

    })
    $(document).on("click","#btnSaveAppointment",(e)=>{
        var task = {
